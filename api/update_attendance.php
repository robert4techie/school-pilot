...
try {
    // Authentication check - make username optional with fallback
    ...
    $username = $_SESSION['username'] ?? $_SESSION['email'] ?? $_SESSION['user_id'] ?? 'Unknown';
    
    $user_id = $_SESSION['user_id'];
    $username = $_SESSION['username'] ?? $_SESSION['email'] ?? 'User_' . $user_id;
    
    // Log for debugging
    error_log("Attendance Update Request - User ID: $user_id, Username: $username");
    ...
    // Rate limiting
    $rate_key = 'attendance_rate_' . $user_id;
    ...
    $allowed_statuses = ['present', 'absent', 'late', 'sick'];
    ...
    $verify_stmt = $conn->prepare("SELECT student_id FROM students WHERE student_id = ? AND status = 'active'");
    ...
    // Check for late arrival threshold (configurable)
    $late_threshold_time = '08:15:00'; // Configure this as needed
    $current_time = date('H:i:s');
    $is_late_arrival = false;
    
    if ($status === 'present' && $date === date('Y-m-d')) {
        // Only check for same day marking
        if ($current_time > $late_threshold_time) {
            $is_late_arrival = true;
            // Optionally auto-change to late
            // $status = 'late';
            // $remarks = empty($remarks) ? 'Arrived after ' . $late_threshold_time : $remarks;
        }
    }
    ...
    try {
        ...
        $lock_stmt->bind_param("ss", $student_id, $date);
        $lock_stmt->execute();
        ...
        if ($existing) {
            ...
            $update_stmt = $conn->prepare(
                "UPDATE attendance 
                 ...
                 WHERE student_id = ? AND date = ? AND version = ?"
            );
            $update_stmt->bind_param("ssisssi", $status, $remarks, $new_version, $user_id, $student_id, $date, $current_version);
            $update_stmt->execute();
            ...
            $insert_stmt = $conn->prepare(
                "INSERT INTO attendance (student_id, date, status, remarks, version, recorded_by, created_at, updated_at) 
                 VALUES (?, ?, ?, ?, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
            );
            $insert_stmt->bind_param("ssssi", $student_id, $date, $status, $remarks, $user_id);
            $insert_stmt->execute();
        ...
        $audit_stmt = $conn->prepare(
            "INSERT INTO attendance_audit 
             (attendance_id, student_id, date, old_status, new_status, changed_by, changed_by_name, change_reason, ip_address, user_agent, changed_at) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)"
        );
        $audit_stmt->bind_param("issssissss", 
            $attendance_id, $student_id, $date, $old_status, $status, 
        ...
        // Log success
        error_log(sprintf(
            "[SUCCESS] User %s (%s) marked attendance: student=%s, date=%s, status=%s, remarks=%s",
            $username, $user_id, $student_id, $date, $status, $remarks ? 'YES' : 'NO'
        ));
    ...
    error_log(sprintf(
        "[ERROR] Attendance update failed: %s | User: %s | IP: %s",
        $e->getMessage(),
