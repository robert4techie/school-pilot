...
// Handle multiple students from checkbox array (from sel_gen_ind_report.php)
...
// Get the number of AOI columns to generate and percentage display option
...
$bulk_mode = false; // Always false for individual reports
...
// Validate that we have students selected
if (empty($student_ids)) {
    echo "<div class='alert alert-danger' style='margin: 50px; padding: 20px;'>
            <h4>No Students Selected</h4>
            <p>Please go back and select at least one student to generate reports.</p>
            <a href='sel_gen_ind_report.php' class='btn btn-primary'>Go Back</a>
          </div>";
    exit;
}
...
// Function to convert marks to grade (for identifier in this report)
function convertToGrade($marks)
{
    ...
    // For assessment report, this is typically the 3-point identifier scale
    if ($marks >= 2.5) return '3'; // Outstanding
    if ($marks >= 1.5) return '2'; // Moderate  
    if ($marks >= 0.9) return '1'; // Basic
    return '0'; // Below basic range / Not achieved
}
...
<head>
    ...
    <title>Assessment Reports - " . count($student_ids) . " Student" . (count($student_ids) > 1 ? 's' : '') . " - {$class} - {$term} {$year}</title>
   ...
   <style>
    ...
    .student-photo {
        max-width: 90px; /* CHANGED: Slightly larger photo. */
        height: auto;
    }
    ...
    .comments-table td {
        text-align: left;
        padding: 10px 12px; /* CHANGED: Increased padding. */
        font-size: 12px; /* CHANGED: Increased font size. */
        vertical-align: top; /* ADDED: Aligns content to the top of the cell. */
    }
    
    /* ADDED: New rule to create space for the Head Teacher's stamp */
    .comments-table tr:last-child td {
        padding-bottom: 50px; /* This adds 50px of space at the bottom of the last row */
    }
...
.aoi-back-page .school-name {
    font-size: 16px !important; /* Reduced from larger size */
    margin-bottom: 3px;
}
...
.aoi-back-page > div:last-child table td {
    padding: 4px 5px !important; /* Reduced from 6px */
...
.aoi-table.large-content th {
    font-size: 9px !important; /* Reduced from 11px */
    padding: 3px 4px !important; /* Reduced from 5px */
...
.aoi-table.extra-large-content th {
    ...
    padding: 2px 3px !important; /* Reduced from 4px */
...
<body>
    <div class='print-btn-container'>
        <button class='btn btn-primary btn-sm' onclick='window.print();'>
            <i class='fas fa-print'></i> Print Reports (" . count($student_ids) . " student" . (count($student_ids) > 1 ? 's' : '') . ")
        </button>
    </div>

    <div class='student-counter'>
        Selected Students: " . count($student_ids) . "
    </div>
...
foreach ($student_ids as $sid) {
    ...
    // First, identify which subjects will be displayed
    ...
    foreach ($displayed_subjects as $subj) {
        ...
        // Students who do fewer AOIs should get proportionally lower scores
...
foreach ($sorted_student_ids as $student_id_from_loop) {
    ...
    generateAssessmentReport($student, $all_marks, $selected_topics, $subject_names_cache, $compulsory_cache, $teacher_initials_cache, $conn, $position_text, $term, $year, $class, $all_students_marks[$student_id_from_loop]['stream'] ?? '', $aoi_columns, $display_percentage, $display_student_position, $school_details);
...
function generateAssessmentReport($student, $all_marks, $selected_topics, $subject_names_cache, $compulsory_cache, &$teacher_initials_cache, $conn, $position_text, $term, $year, $class, $stream, $aoi_columns, $display_percentage, $display_student_position, $school_details)
{
    ...
    <div class='col-8 school-info'>
        ...
        <div class='mt-2'>
            <strong>ASSESSMENT REPORT CARD FOR TERM " . htmlspecialchars($term) . ", YEAR: " . htmlspecialchars($year) . "</strong>
        </div>
    </div>
            <div class='col-2'>";
    ...
    echo "
                        <th rowspan='2' class='grade-column calculated'>IDENTIFIER</th>
    ...
    if ($display_student_position && $position_text !== null) {
        ...
        echo "<td style='width: 25%;'>Final Identifier: <span id='final-grade-{$internal_student_id}'>-</span></td>"; // Use internal_student_id
        ...
        echo "<td style='width: 33.33%;'>Final Identifier: <span id='final-grade-{$internal_student_id}'>-</span></td>"; // Use internal_student_id
    ...
    foreach ($selected_topics as $subject => $topics) {
        ...
        if (!empty($aoi_marks_check)) {
            ...
            $aoi_avg = $aoi_sum / $aoi_columns; // Divide by total columns, not count
    ...
    <table class='grades-table'>
        ...
        <tr>
            <td style='font-weight: bold;'>DESCRIPTOR</td>
            <td>Basic</td>
            <td>Moderate</td>
            <td>Outstanding</td>
        </tr>
...
<script>
    (function() {
        ...
        function calculateGrade(marks) {
            ...
            // Assessment report grading scale (out of 3)
            if (marks >= 2.5) return '3';
            if (marks >= 1.5) return '2';
            if (marks >= 0.9) return '1';
            return '0'; // Below basic range
        }
    ...
    <script>
        ...
        setTimeout(function() {
            ...
            allTables.forEach(table => {
                const event = new Event('recalculate'); // This event is not currently handled but is harmless
